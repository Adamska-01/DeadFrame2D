...
namespace DF2D::Factories
{
	...
	GraphicsBackendSet GraphicsBackendFactory::CreateProduct(const WindowConfig& windowConfig, const RendererConfig& rendererConfig)
	{
		...
		auto renderBackend = std::make_unique<SDLRenderBackend>(windowBackend->GetSDLWindow(), rendererConfig);
